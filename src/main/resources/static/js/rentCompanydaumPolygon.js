...
kakao.maps.event.addListener(map, 'rightclick', function (mouseEvent) {
    ...
    if (drawingFlag) {
        ...
        var path = String(polygon.getPath()).replace(/\)\,\(/gi, ':').replace(/\, /gi, '|');
        path = path.replace(/\(/gi, 'LAT');
        path = path.replace(/\)/gi, '');
        path = path.replace(/\|/gi, '|LNG');
        path = path.replace(/\:/gi, ',LAT');
        path = path.replace(/LNG/gi, 'LNG:');
        path = path.replace(/LAT/gi, 'LAT:');
        $("#latlng").text(path);
        ...
        if (path.length > 2) {
            ...
            content = '<div class="info">총면적 <span class="number"> ' + area + '</span> m<sup>2</sup><br>"배달위치 저장" 버튼 클릭 하여 저장해 주세요.</div>'; // 커스텀오버레이에 추가될 내용입니다
//            content = '<div class="info">저장 버튼 클릭 하여 주세요.</div>'; // 커스텀오버레이에 추가될 내용입니다
...
function PolygList() {
	...
	var polygonPath = [];
	//push
	//polygonPath.push(new kakao.maps.LatLng(37.50037137207458,127.03745546740966));
	//polygonPath.push(new kakao.maps.LatLng(37.48177267105687,127.04305406353073));
	//polygonPath.push(new kakao.maps.LatLng(37.506706548378496,127.05718322480274));
	//polygonPath.push(new kakao.maps.LatLng(37.48940261134505,127.06603500381657));
	
	let response = {};
	let reqParam = {};
	var ragbncode = sessionStorage.getItem('raGbnCode');
	var _rtIdx = sessionStorage.getItem('rtIdx');
	reqParam = {
			'rtIdx' : _rtIdx
			,'raGbnCode' : ragbncode
		};
	...
	let url = "/api/v1.0/selectDcRentCompanyAblearea.json";
	...
	$.ajax({
	    ...
	    timeout: 1000
	 		...
	 		response = data;
	 		let resmap = response.data.result;

			for ( let i in resmap) {
				polygonPath.push(new kakao.maps.LatLng(resmap[i].lat,resmap[i].lng));
			}
...
function Polygonsave() {
	var ragbncode = sessionStorage.getItem('raGbnCode');
	var _rtIdx = sessionStorage.getItem('rtIdx');
	var adminidx = sessionStorage.getItem('adminidx');
	...
	if (ragbncode == 'L' || ragbncode == 'S'){
		...
		let url = "/api/v1.0/insertDcRentCompanyAblearea.do";
		...
		$.ajax({
		    ...
		    timeout: 1000
		 		...
		 		sessionStorage.setItem('companyAddress', '*');
		 		sessionStorage.setItem('raGbnCode', '*');
		 		sessionStorage.setItem('rtIdx', '*');
		 	});
	}
}
